...
public class CustomerController {
    ...
    void loginCustomer(String customername,
                   String password,
                   String customertype)  {
        log.info("执行登录");
    }
    ...
    @ApiOperation(value = "客户退出",notes = "客户退出")
    ResponseWrap<Boolean> logoutCustomer()  {
        return ResponseWrapBuild.build(CustomerEnum.LOGOUT_SUCCEED,true);
    }
     ...
     * @return
     */
    @GetMapping("role-by-customerId/{customerId}")
    @ApiOperation(value = "查找客户有角色",notes = "查找客户有角色")
    @ApiImplicitParam(paramType = "path",name = "customerId",value = "客户ID",example = "1",dataType = "Long")
     ...
     * @return
     */
    @GetMapping("resource-by-customerId/{customerId}")
